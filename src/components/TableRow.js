...
const TableRow = (props) =>
{
  ...
  return(
    <div className="table-row-container" style={{color: getColor()}}>
      ...
      <div className="table-row table-row-upper" onClick={() => setShowDescription(!showDescription)} id={props.courseCode}>

        <div style={{flex: "0 0 80px"}} className="table-row-element">{props.courseCode}</div>

        <div style={{flex: "0 0 25px"}} className="table-row-element">{props.hp}</div>

        {
          props.year
          &&
          <div style={{flex: "0 0 35px"}} className="table-row-element">{props.year}</div>
        }

        <div style={{width: "100%"}} className="table-row-element">{getTitle()}</div>

        <div className="table-row-element" id="field-element">{getField()}</div> 
